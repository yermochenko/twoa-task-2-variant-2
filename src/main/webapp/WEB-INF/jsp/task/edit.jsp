...
<u:page title="${title}">
	...
	<form action="${url_task_save_html}" method="post" class="w3-container form">
		...
		<div class="form-element">
			<table>
				...
				<tr>
					<th colspan="2">Проверяемая точка</th>
					<td colspan="6" rowspan="3"></td>
				</tr>
				<tr>
					<td><label for="x0"><span class="math"><i>x</i><sub>0</sub></span>:</label></td>
					<td><input type="text" name="x0" value="${task.point.x}" id="x0" class="w3-input w3-border"></td>
				</tr>
				<tr>
					<td><label for="y0"><span class="math"><i>y</i><sub>0</sub></span>:</label></td>
					<td><input type="text" name="y0" value="${task.point.y}" id="y0" class="w3-input w3-border"></td>
				</tr>
			</table>
		</div>
	...
	<c:if test="${not empty task and not empty task.pointTriangleRelativePosition}">
		<h3>Решение задачи</h3>
		<c:choose>
			<c:when test="${task.pointTriangleRelativePosition == 'INSIDE'}">
				<p>Точка лежит <span id="point_triangle_relative_position">внутри</span> треугольника.</p>
			</c:when>
			<c:when test="${task.pointTriangleRelativePosition == 'OUTSIDE'}">
				<p>Точка лежит <span id="point_triangle_relative_position">снаружи</span> треугольника.</p>
			</c:when>
			<c:when test="${task.pointTriangleRelativePosition == 'ONSIDE'}">
				<p>Точка лежит <span id="point_triangle_relative_position">на стороне</span> треугольника.</p>
			</c:when>
			<c:when test="${task.pointTriangleRelativePosition == 'VERTEX'}">
				<p>Точка <span id="point_triangle_relative_position">совпадает с вершиной</span> треугольника.</p>
			</c:when>
		</c:choose>
	</c:if>
</u:page>